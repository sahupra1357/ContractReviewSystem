...
def test_dev_environments_are_exempt(environment):
    ...
    assert deployment_config_problems(Settings(environment=environment)) == []


def test_fully_configured_deployment_passes():
    assert deployment_config_problems(Settings(**DEPLOYED)) == []


def test_unconfigured_deployment_reports_every_problem():
    problems = deployment_config_problems(Settings(environment="render-free"))
...
def test_dev_jwt_secret_alone_is_flagged():
    ...
    settings = Settings(**{**DEPLOYED, "jwt_secret": "dev-secret-change-me"})
...
def test_openai_embeddings_require_a_key():
    problems = deployment_config_problems(
        Settings(**DEPLOYED, embedding_provider="openai")
    )
    assert any("CRS_EMBEDDING_API_KEY" in p for p in problems)

    ok = deployment_config_problems(
        Settings(**DEPLOYED, embedding_provider="openai", embedding_api_key="sk-x")
    )
    assert ok == []


def test_bge_m3_needs_no_embedding_key():
    assert deployment_config_problems(Settings(**DEPLOYED, embedding_provider="bge-m3")) == []


def test_require_raises_with_actionable_message():
    with pytest.raises(RuntimeError) as exc:
        require_valid_deployment_config(Settings(environment="render-free"))
...
def test_require_is_silent_when_configured():
    require_valid_deployment_config(Settings(**DEPLOYED))
